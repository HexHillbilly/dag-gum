#!/usr/bin/env python3
"""Phase 2 benchmark for semantic-style-proxy (SSP): prompt-injection vs logit-bias.

Adds a `logit_bias` condition (decode-time corpus style-prior, see
style_prior.py) and sweeps its valve λ, head-to-head against the prompt-injection
conditions. Adds `corpus_ppl` (perplexity of the output under the corpus n-gram
model) as the fingerprint metric — lower = closer to target. Adds a `style`
condition (SSP Phase A) — the "write like X" voice, the product under test.
...
Conditions:
  ...
  style             SSP Phase A: "write like X" voice + top-1 line (wrap_style)
...
NGRAM_N = 2
TRAIN_RATIO = 0.8  # held-out split: index/retrieval + n-gram on train, style_cos on test
...
# Prose-writing probes for the style-transfer test. The Daisy probes above are
# conversational (some even poetry/persona), which dilutes a style signal — the
# register is forced by the task, not the target voice. These are open, style-
# neutral narration/description prompts where the authorial voice IS the signal.
STYLE_PROBES = [
    "Describe a morning in a small town.",
    "Tell me about a meal you once had.",
    "Describe walking along a river at dawn.",
    "What happened on your way home last night?",
    "Describe a place you remember well.",
    "Tell about a time you were afraid.",
    "Describe making a cup of coffee.",
    "What did you see when you looked out the window?",
    "Describe a long journey by road.",
    "Tell about a fight you once witnessed.",
]

BASE_CONDITIONS = ["baseline", "markov", "rag", "persona", "style", "style_rhythm"]
...
def build_condition(condition, user_msg, retrieve_fn, ngram_model, persona_name, persona_desc, style_name, style_desc, tok, rhythm_min, rhythm_max, rhythm_penalty):
    ...
    if condition == "persona":
        return wrap_persona_hardened(user_msg, matched[0], persona_name, persona_desc), {
            "retr_lines": matched,
            "retr_l2": dists,
        }, []
    if condition == "style":
        return wrap_style(user_msg, matched[0], style_name, style_desc), {
            "retr_lines": matched,
            "retr_l2": dists,
        }, []
    if condition == "style_rhythm":
        return wrap_style(user_msg, matched[0], style_name, style_desc), {
            "retr_lines": matched,
            "retr_l2": dists,
        }, [SentenceRhythmProcessor(tok, min_tokens=rhythm_min, max_tokens=rhythm_max, penalty=rhythm_penalty)]
...
def main():
    ...
    ap.add_argument(
        ...
        default="daisy",
        choices=["daisy", "style"],
        help="daisy=conversational probes, style=prose-writing probes",
    )
    ap.add_argument(
        "--train-ratio",
        type=float,
        default=TRAIN_RATIO,
        help="fraction of corpus for index/retrieval; rest = held-out style_cos centroid",
    )
    ap.add_argument("--rhythm-min", type=int, default=4, help="style_rhythm min sentence tokens")
    ap.add_argument("--rhythm-max", type=int, default=40, help="style_rhythm max sentence tokens")
    ap.add_argument("--rhythm-penalty", type=float, default=20.0, help="style_rhythm end-token penalty")
    ap.add_argument("--persona-name", default="Daisy")
    ap.add_argument(
        "--persona-desc",
        default="a warm, curious, slightly naive conversationalist",
    )
    ap.add_argument("--style-name", default="Ernest Hemingway")
    ap.add_argument(
        "--style-desc",
        default="spare, declarative prose with short sentences, concrete nouns and verbs, and no ornament",
    )
    ap.add_argument(
        "--conditions",
        default=None,
    ...
    probes = PROBES if args.probe_set == "daisy" else STYLE_PROBES
    ...
    if args.conditions:
        ...
        conds = [
            c
            for c in CONDITIONS
            if c in want or (c.startswith("logit_bias_") and "logit_bias" in want)
        ]
    ...
    # Held-out split: index/retrieval + n-gram model on train; style_cos centroid
    # on held-out test lines. This kills the echo confound — retrieval no longer
    # scores against the same sentences it feeds back into the prompt.
    rng = random.Random(SEED)
    idxs = list(range(len(corpus)))
    rng.shuffle(idxs)
    n_train = max(1, min(int(len(idxs) * args.train_ratio), len(idxs) - 1))  # >=1 train, >=1 test
    train_corpus = [corpus[i] for i in idxs[:n_train]]
    test_corpus = [corpus[i] for i in idxs[n_train:]]
    index = build_index(train_corpus, embedder)
    corpus_centroid = embed(embedder, test_corpus).mean(axis=0)
    print(f"[*] Held-out split: {len(train_corpus)} train / {len(test_corpus)} test")
    ...
    ngram_model = build_ngram_model(tok, train_corpus, n=NGRAM_N)
    print(f"[*] Corpus n-gram model: {len(ngram_model)} contexts")

    def retrieve_fn(prompt):
        return retrieve(prompt, index, train_corpus, embedder, k=3)
    ...
    for probe in probes:
        ...
        for cond in conds:
            prompt_wrapped, diag, processors = build_condition(
                ...
                args.style_name, args.style_desc, tok,
                args.rhythm_min, args.rhythm_max, args.rhythm_penalty,
            )
