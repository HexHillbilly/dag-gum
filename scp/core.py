...
def wrap_persona_topic(
    user_msg: str,
    persona_line: str,
    name: str = "Daisy",
    desc: str = "a warm, curious, slightly naive conversationalist",
) -> str:
    """v3 directive: persona voice WITHOUT topic bleed. Adds an explicit
    topic boundary so the seed line's *tone* is adopted but its *subject* is
    not imported into the answer (the Phase 3b finding: narrow-corpus retrieval
    contaminated general questions)."""
    return (
        f"You are {name}, {desc}. "
        f"A line in your own voice: '{persona_line}'. "
        f"Match its tone and voice in your reply, but answer only the question's "
        f"own topic — do not bring in the line's subject matter.\n\n"
        f"Request: {user_msg}"
    )


def wrap_explain_in_terms(
    user_msg: str,
    persona_line: str,
    name: str = "Daisy",
    desc: str = "a warm, curious, slightly naive conversationalist",
) -> str:
    """v4 directive: explicit "explain in their terms" framing.

    The topic-bleed mechanism, read as a feature: demand BOTH the explanation
    (topic anchored to the request) AND the persona voice (their terms), so the
    seed line's lexicon transfers while the answer still addresses the question.
    Contrast with v2's "weave its spirit" (voice-only, which occasionally drops
    the explanation) and v3's "answer only the question's topic" (topic-only,
    which killed the voice)."""
    return (
        f"You are {name}, {desc}. "
        f"A line in your own voice: '{persona_line}'. "
        f"Explain the request below thoroughly and accurately, in your own "
        f"{name} voice and terms.\n\n"
        f"Request: {user_msg}"
    )
