# agent-036 — Temporal Hardening Sweep (Option B) — MEASUREMENT REPORT
...
| Arms | baseline (agent-034e), P4 (spectral persistence >= 4), S30 (step >= 30) |
| Fixtures | prose-100 (heldout partition), t2s detection-gap 93 (recall) |
| band_low | 8.216097 (frozen) |

## Dispatch gate

- agent-035 (post-B2) sampling rescue: t2s 97/100, qwen 97/100 (bar 85/100 MET) -> agent-036 dispatchable.
- agent-033 detection-gap classification: reproducible from `docs/gate23/production_cross_fixture_results.jsonl` (`diagnostic_class == "detection-gap"`, 93 records).
...
|---|---|---|---|
| baseline (agent-034e) | 0/100 | 0/100 | 39/93 (0.4194) |
...
- **5a (FP):** sampling baseline is already **0/100** — the "2 late-dip FPs"
  (agent-034c/d/e records 98, 99) do NOT occur under sampling. Neither arm
...
"short macro-loop" recall tax the independent critique warned about.

## Key finding

The two spectral late-dip FPs that motivated agent-036 are a **greedy-only
...
(sampling) regime — and it costs recall. Recommend: leave the agent-034e rule
...
- No controller diff landed; no threshold changed. Measurement harness:
  `scripts/measure_agent036_temporal_hardening.py` (subclass override of
  `_evaluate_collapse`).
