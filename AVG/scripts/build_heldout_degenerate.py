#!/usr/bin/env python3
"""DS-025: Gate 2.3 held-out hard-degenerate slice (Part B).
...
``docs/gate23/FROZEN_THRESHOLDS.md`` (Part A) and are consumed by
``scripts/score_gate23.py`` (Part C).
...
Method (per the DS-025 task file / greenlight):
  1. Held-out prose prompts = the COMPLEMENT of the Gate 2.2 prose control 100
     ...
     ds-020 / ds-023 / ds-024; the heldout slice is the other 100).
  2. For each of the 100 held-out prompts (first 40 whitespace tokens of the
     prose text), run a deterministic decoding grid:
         do_sample=True, top_k=40, top_p=1.0 (disabled),
         temperature in {1.0, 1.4}, repetition_penalty=1.0,
         max_new_tokens=min_new_tokens=128.
     Grid size = 100 prompts x 2 cells = 200 records.
  3. Curate the 100 grid records with the LOWEST last-24-token Distinct-2
  ...
  4. Determinism smoke FIRST: two grid records regenerated with identical
     seeds must produce byte-identical continuations, otherwise STOP and
     report.

Seed provenance: SEED = 4244 (DS-025). Every grid generation uses a unique
deterministic seed derived from SEED, the heldout sample index, and the
grid-cell index (SEED*1000 + sample_idx*10 + cell_idx), so the full 200-record
...
# repetition_penalty=1.0 means no anti-repetition pressure; top_k=40;
# top_p=1.0 disables nucleus sampling so the grid is exactly the greenlight's
# top_k=40 x temp {1.0,1.4} specification).
GRID: List[Dict[str, Any]] = [
    {"temperature": 1.0, "top_k": 40},
    {"temperature": 1.4, "top_k": 40},
]
GRID_SAMPLE = True
GRID_TOP_P = 1.0
...
OUTPUT_PATH = ROOT / "tests" / "fixtures" / "heldout_degenerate.jsonl"
REPORT_PATH = ROOT / "docs" / "gate23" / "HELDOUT_PROFILE.md"
...
def run_determinism_smoke(
    ...
    for sample_idx, cell_idx in SMOKE_POINTS:
        ...
        a = run_generation(
            model, tokenizer, prompt, cell["temperature"], cell["top_k"],
            seed, device,
        )
        b = run_generation(
            model, tokenizer, prompt, cell["temperature"], cell["top_k"],
...
def build_grid(
    ...
    """Generate the full 100x2 grid and return 200 flat records.

    Each record carries the top-level fixture keys: id (assigned by curate),
    source_prose_id, prompt, params, text, distinct_2.
    """
    records: List[Dict[str, Any]] = []
    ...
    for sample_idx, rec in enumerate(heldout_prompts):
        ...
        for cell_idx, cell in enumerate(GRID):
            ...
            if n % 50 == 0 or n == total:
                print(f"[ds-025] grid generation {n}/{total}", flush=True)
    return records
...
def markdown_table(corpus: Dict[str, float], prose: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"|--------|------|--------|-----|\n"
        f"| heldout_degenerate (curated) | {corpus['mean']:.4f} | "
...
def write_profile(
    ...
    """Write docs/gate23/HELDOUT_PROFILE.md (strictly factual)."""
    md: List[str] = []
    md.append("# DS-025 — Held-Out Hard-Degenerate Slice (Part B)")
    md.append("")
    ...
    md.append("")
    md.append("All grid cells use `do_sample=True`, `top_k=40`, `top_p=1.0` ")
    md.append("(disabled), and `repetition_penalty=1.0` (no anti-repetition ")
    md.append("pressure).")
    md.append("")
    md.append("| # | temperature | top_k | top_p |")
    md.append("|---|------------|-------|-------|")
    for cell_idx, cell in enumerate(GRID):
        md.append(f"| {cell_idx} | {cell['temperature']} | {cell['top_k']} | {GRID_TOP_P} |")
    md.append("")
    ...
    md.append("100 of the 200 certified valid_subset_200 records were selected as "
             "the Gate 2.2 prose control with "
    ...
    md.append("## Fixture")
    md.append("")
    md.append("- JSONL: `tests/fixtures/heldout_degenerate.jsonl`")
...
# ---------------------------------------------------------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description="DS-025 held-out degenerate fixture")
    ...
    print("=" * 70)
    print("DS-025 Gate 2.3 held-out degenerate slice (Part B)")
    print("=" * 70)
    print(f"SEED: {SEED}")
    ...
    if args.skip_smoke:
        ...
        prompts = [first_n_whitespace_tokens(r["text"]) for r in heldout]
    ...
    # Write outputs
    # ------------------------------------------------------------------
    # Renumber fixture ids 0..99 in curation order.
    ...
    metadata = {
        ...
        "Prompt truncation": f"first {PROMPT_WHITESPACE_TOKENS} whitespace tokens",
