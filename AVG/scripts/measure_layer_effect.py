#!/usr/bin/env python3
"""DS-026: Joint detection x effect layer measurement (MEASUREMENT ONLY).
...
ds-025 (Gate 2.3 Part A) froze per-layer detection thresholds
(docs/gate23/FROZEN_THRESHOLDS.md); ds-027 built the heldout-degenerate v2
fixture (tests/fixtures/heldout_degenerate_v2.jsonl). This probe measures the
intervention EFFECT per candidate layer {2, 12, 26} on that fixture.
...
Method (per the DS-026 task file):
  ...
  2. Per candidate layer L in {2, 12, 26}, per record, TWO greedy
     ...
     (a) dormant: no hooks;
     (b) hooked: a forward hook on layer L that, when PR(L) over the trailing
         24 generated positions falls below the frozen T_PR(L), applies ONE
         bounded orthogonal-style perturbation (0.45 L2, clamp [0.35, 0.75])
         to the current hidden state, then clears.
  3. Measure per record per layer: intervention count, applied-force L2
     stats, continuation distinct_2 (last-24 window) and CTR (hooked vs
     dormant), and mean next-token KL divergence between hooked and dormant
     logits at the 8 generation steps following each application.
...
Environment notes (identical to ds-025):
...
# ---------------------------------------------------------------------------
SEED = 42  # DS-026 measurement seed (matches ds-025 / ds-024 replay seed)
...
# KL measurement horizon: 8 generation steps following each application.
KL_STEPS = 8
...
OUTPUT_JSONL = Path("docs/gate23/layer_effect_results.jsonl")
OUTPUT_MD = Path("docs/gate23/LAYER_EFFECT_RESULTS.md")
...
# ---------------------------------------------------------------------------
# Greedy generation with optional measurement hook
# ---------------------------------------------------------------------------
@torch.no_grad()
def greedy_generate(
    ...
    When ``layer``/``pr_threshold`` are given, registers a forward hook on
    ``model.model.layers[layer]``. The hook computes PR over the trailing 24
    generated positions at each decoding step; when PR < T_PR(L), it applies
    ONE bounded orthogonal-style perturbation (0.45 L2, clamp [0.35, 0.75]) to
    the current hidden state, then clears (fires at most once).
    ...
    state: Dict[str, Any] = {
        "fired": False,
        "fire_decoding_step": None,
        "pr_at_fire": None,
        "l2_before_clamp": None,
        "l2_after_clamp": None,
    }
            ...
            if hs.shape[1] == 1:
                ...
                if len(buffer) >= ANALYSIS_WINDOW:
                    ...
                    if pr < pr_threshold:
                        ...
                        l2_before = float(BASE_STRENGTH)
                        clamped_force = max(
                            FORCE_CLAMP_LOW, min(FORCE_CLAMP_HIGH, BASE_STRENGTH)
                        )
                        l2_after = float(clamped_force)
                        intervened = original.clone()
                        intervened[:, -1, :] = (
                            res_flat + clamped_force * ortho_unit
                        ).to(dtype=orig_dtype)
                        state["fired"] = True
                        state["fire_decoding_step"] = int(counter["decoding_step"])
                        state["pr_at_fire"] = float(pr)
                        state["l2_before_clamp"] = l2_before
                        state["l2_after_clamp"] = l2_after
                        return intervened
            return output
...
def mean_kl_after_fire(
    ...
    """
    S = state.get("fire_decoding_step")
    if S is None:
...
def write_markdown_report(ctx: Dict[str, Any], pr_thr: Dict[str, float]) -> None:
    md: List[str] = []
    md.append("# DS-026 — Layer effect measurement (per-layer intervention effect)")
    md.append("")
    ...
    md.append("| Perturbation | orthogonal-style, "
              f"{meta['force_budget']['base_strength']} L2, clamp "
              f"{meta['force_budget']['clamp']} (controller.py:120-146,566) |")
    ...
    md.append("- Dormant: no hooks. Hooked: a forward hook on layer L computes")
    md.append("  PR over the trailing 24 generated positions at each decoding")
    md.append("  step; when PR < T_PR(L) (frozen), it applies ONE bounded")
    md.append("  orthogonal-style perturbation (0.45 L2, clamp [0.35, 0.75]) to")
    md.append("  the current hidden state, then clears (at most one fire).")
    ...
    md.append("- KL is the mean next-token KL(hooked || dormant) over the 8")
    md.append("  generation steps following each application (the application")
    md.append("  step's forward pass and the next 7). Where logits could not be")
    md.append("  captured cleanly it is reported as NOT MEASURED.")
    md.append("")
    ...
    md.append(format_table_md(
        rows, ["layer", "n", "dorm mean", "dorm med", "hook mean", "hook med",
               "Δ mean", "Δ med", "#Δ>0", "#fired"],
    ))
    md.append("")
    md.append("- #Δ>0 = number of records where hooked distinct-2 > dormant")
    md.append("  distinct-2; #fired = number of records where the hook fired.")
    md.append("")
    ...
    # ---- force stats ----
    md.append("## Applied-force L2 stats (per layer, records where the hook fired)")
    md.append("")
    md.append("The perturbation is a single 0.45 L2 orthogonal-style impulse")
    md.append("(production base_strength), clamped to [0.35, 0.75]. Because the")
    md.append("base strength is inside the clamp, before/after clamp are equal.")
    md.append("")
    rows = []
    for L in CANDIDATE_LAYERS:
        s = by_layer[L]
        rows.append([
            L, str(s["n_fired"]),
            fmt(s["l2_before"]["mean"]), fmt(s["l2_after"]["mean"]),
            fmt(s["fire_step"]["min"]), fmt(s["fire_step"]["median"]),
            fmt(s["fire_step"]["max"]),
            fmt(s["pr_at_fire"]["mean"]), fmt(s["pr_at_fire"]["min"]),
        ])
    md.append(format_table_md(
        rows, ["layer", "n_fired", "L2 before", "L2 after",
               "fire step min", "fire step med", "fire step max",
               "PR@fire mean", "PR@fire min"],
    ))
    md.append("")

    # ---- KL table ----
    md.append("## Mean next-token KL (hooked vs dormant, 8 steps after each fire)")
    md.append("")
    md.append("KL(hooked || dormant) over the 8 generation steps following the")
    md.append("application (the application step's forward pass plus the next 7).")
    md.append("Per-record KL is averaged over those steps; the table aggregates")
    md.append("per layer over records where the hook fired and KL was measured.")
    md.append("")
    rows = []
    for L in CANDIDATE_LAYERS:
        s = by_layer[L]
    ...
    md.append(format_table_md(
        rows, ["layer", "n_fired+KL", "note", "mean", "median", "min", "max"],
    ))
    md.append("")
    ...
    md.append("intervention count, force L2, per-fire KL) are in")
    md.append("`layer_effect_results.jsonl`.")
    md.append("")

    md.append("## Notes / caveats")
    md.append("")
    md.append("- MEASUREMENT ONLY. No verdict, no layer recommendation.")
    md.append("- The hook fires at most once per record per layer (spec: ONE")
    md.append("  perturbation, then clears).")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DS-026 layer effect measurement (measurement only)"
    )
    ...
    print("=" * 70)
    print("DS-026 Layer effect measurement (MEASUREMENT ONLY)")
    print("=" * 70)
    print(f"SEED: {SEED}")
    print(f"device: {device} | dtype: {dtype}")
    print(f"model: {MODEL_NAME}@{MODEL_REVISION}")
    print(f"torch: {torch.__version__}")
    print(f"force budget: base={BASE_STRENGTH} max={MAX_STRENGTH} "
          f"clamp=[{FORCE_CLAMP_LOW},{FORCE_CLAMP_HIGH}]")
    ...
    for rec_i, rec in enumerate(records):
        ...
        for layer_str in CANDIDATE_LAYERS:
            ...
            st = h["state"]
            kl = mean_kl_after_fire(st, h["logits"], dorm["logits"])
            ...
            rec_out = {
                ...
                "hooked": {
                    ...
                    "intervention_count": 1 if st["fired"] else 0,
                    "interventions": [
                        {
                            "step": st["fire_decoding_step"],
                            "pr_at_fire": st["pr_at_fire"],
                            "l2_before_clamp": st["l2_before_clamp"],
                            "l2_after_clamp": st["l2_after_clamp"],
    ...
    for L in CANDIDATE_LAYERS:
        ...
        l2_before = [
            i["l2_before_clamp"]
            for r in fired for i in r["hooked"]["interventions"]
        ]
        l2_after = [
            i["l2_after_clamp"]
            for r in fired for i in r["hooked"]["interventions"]
        ...
        fire_steps = [
            i["step"] for r in fired for i in r["hooked"]["interventions"]
        ]
        pr_at_fire = [
            i["pr_at_fire"] for r in fired for i in r["hooked"]["interventions"]
        ]
        kl_means = [
            i["kl_8step_mean"] for r in fired
            for i in r["hooked"]["interventions"]
        ...
        by_layer[L] = {
            ...
            "n_fired": len(fired),
            "l2_before": summarize(l2_before),
            "l2_after": summarize(l2_after),
