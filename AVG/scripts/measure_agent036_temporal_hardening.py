#!/usr/bin/env python3
"""agent-036 — Temporal Hardening Sweep (Option B) — MEASUREMENT.

Sweeps two temporal-hardening candidates against the committed agent-034e
...
generate() path, only the spectral threshold logic diverges):
  ...
  baseline : spectral persistence >= 2, no step floor (committed agent-034e)
...
Writes batch/queue/agent-036-temporal-hardening-results.jsonl
"""
import gc
import json
import random
...
CONTROL_N = 100
BAND_LOW = 8.216097
OUT = "batch/queue/agent-036-temporal-hardening-results.jsonl"
...
def main():
    ...
    print("=== Dispatch gate: agent-035 (post-B2) result ===")
    print("  agent-035 sampling rescue: t2s 97/100, qwen 97/100 (success bar 85/100 MET) -> agent-036 dispatchable")
    print("  Detection-gap classification (agent-033): reproducible from", CLASS_FILE)
