#!/usr/bin/env python3
"""night-006: Controller integration probe — dual-predicate + spectral-gated
kickstart (GATE).

night-005 proved that the dual-predicate's actuation role must be NARROW:
production unconditional suppression stays unchanged, and the dual-predicate
adds only spectral-gated kickstart for macro-loop redirect. This probe tests
the exact `diagnose()` / `generate()` changes proposed for
...
Proposed detection (dual-predicate diagnose() logic, exact):
...
Proposed actuation (generate() changes, exact):
    ...
    # NEW: spectral-gated kickstart for macro-loop redirect only
    if spectral_fire and trailing_ctr < 0.50 and active_loop_ids:
        kickstart_counter = 3

The kickstart gate is the ONLY actuation change vs night-004 (which used the
production trigger `trailing_ctr < 0.50 AND active_loop_ids`). Suppression
remains unconditional, matching the production combined path
...
Reused data (do NOT re-run):
  ...
  - DS-033 production rescue rates (A/B) and DS-035 gated rescue rates:
    REUSED for the comparison tables (do NOT re-run).
  - Fixture C production baseline: DS-031 suppression-only 48/50 = 0.9600.
...
# ---------------------------------------------------------------------------
SEED = 42  # night-006 measurement seed (matches ds-025..ds-035, night-004)
...
# Gate criteria (night-006 task file; all four must be met).
...
OUTPUT_JSONL = Path("docs/gate23/controller_integration_results.jsonl")
OUTPUT_MD = Path("docs/gate23/CONTROLLER_INTEGRATION_RESULTS.md")
...
# ---------------------------------------------------------------------------
# KV-cache generation: active (night-006 controller integration probe)
# ---------------------------------------------------------------------------
@torch.no_grad()
def greedy_generate_active_controller_integration(
    ...
    """Greedy (do_sample=False) active generation under the night-006 probe.
    ...
    Proposed actuation (generate() changes, exact):
      ...
      - Spectral-gated kickstart (THE NIGHT-006 CHANGE): fires ONLY when
        spectral_fire AND trailing_ctr < 0.50 AND active_loop_ids (counter=3,
        -1e4/-5.0/-2.0). This narrows the actuation role of the dual-predicate
        to macro-loop redirect only, per the night-005 finding.
    ...
    try:
        for step in range(max_new_tokens):
            ...
            # ---- 6. Actuation: NEW spectral-gated kickstart ----
            # night-005 finding: the dual-predicate's actuation role is NARROW.
            # Suppression needs no predicate gate (production already works);
            # the dual-predicate adds only spectral-gated kickstart for
            # macro-loop redirect. Kickstart fires only when spectral_fire AND
            # trailing_ctr < 0.50 AND active_loop_ids.
            if spectral_fire and trailing_ctr < CTR_THRESHOLD and active_loop_ids:
...
def rescue_provenance(
    ...
    """Per-fixture provenance of the night-006 rescues vs a baseline rescue set.

    For each fixture in baseline_rescued:
      - overlap       : records rescued by BOTH the baseline and night-006.
      - new_rescues   : records rescued ONLY by night-006 (baseline missed).
      - regressions   : records rescued by the baseline but NOT by night-006.
    """
...
def run_determinism_smoke(
    ...
    """Determinism smoke (night-006): one Fixture A record.
      1. Dormant (reused from DS-035 for the same record id), checked for the
         smoke only via a live re-run.
      2. Active (night-006 controller integration), generated twice — token ids
...
def run_determinism_smoke_prose(
    ...
    """Determinism smoke (prose-100): one heldout prose record.
      Active (night-006 controller integration) generated twice — token ids AND
...
def write_markdown_report(ctx: Dict[str, Any]) -> None:
    md: List[str] = []
    md.append("# night-006 — Controller integration probe: dual-predicate + "
              "spectral-gated kickstart (GATE)")
    md.append("")
    md.append("> GATE REPORT. This probe tests the exact `diagnose()` / "
              "`generate()` changes proposed for `governor/controller.py` "
              "BEFORE they reach the controller. Targets are explicit, "
              "pass/fail is binary, a red gate is a STOP signal [1].")
    md.append("")
    ...
    md.append("| Dual-predicate | DS-034e frozen config (proposed diagnose()): "
              "bigram OR spectral, band_low, >=2 consecutive, token_diversity "
              "< 0.40 corroboration on spectral-only fires, code-context "
              "immunity |")
    ...
    md.append("| Kickstart | NEW spectral-gated: spectral_fire AND trailing_ctr "
              "< 0.50 AND active_loop_ids -> counter=3 (-1e4/-5.0/-2.0). "
              "night-005 finding: the dual-predicate's actuation role is NARROW "
              "(macro-loop redirect only). |")
    ...
    # Gate criteria
    md.append("## Gate criteria (quoted from the night-006 task file)")
    md.append("")
    ...
    md.append("One Fixture A record AND one prose-100 record: active "
              "(night-006 controller integration) generated twice. Token ids AND "
    ...
    md.append("")
    md.append("| fixture | target | night-006 | met? |")
    md.append("|---|---|---|---|")
    ...
    md.append("")
    if verdict == "PASS":
        md.append("**PASS** — all four targets met. The proposed controller "
                  "edit (dual-predicate diagnose() + spectral-gated kickstart) "
                  "is validated and can be applied to `governor/controller.py`.")
    else:
    ...
    md.append("DS-033 production rates for Fixtures A/B are REUSED from "
              ...
              "`docs/gate23/dual_predicate_rescue_results.jsonl`. night-004 "
              "unconditional rates are REUSED from "
              "`docs/gate23/unconditional_suppression_results.jsonl`. Fixture C "
    ...
    md.append("Fire counts reflect DUAL-PREDICATE DETECTION liveness. The "
              "dual-predicate does NOT gate suppression in night-006; "
              "suppression is unconditional at every step. The dual-predicate "
              "gates ONLY the spectral kickstart.")
    md.append("")
    ...
    md.append("The ratified RFC-004 A3 FP policy documents 7 FPs (5 bigram "
              ...
              "The gate requires the night-006 probe to not exceed 7 FPs under "
              "the same criterion.")
    md.append("")
    ...
    if ctx.get("provenance_prod"):
        ...
        md.append("")
        md.append("For Fixtures A/B, night-006 rescues are decomposed against "
                  "the DS-033 production rescued record-id set (REUSED).")
        md.append("")
        md.append("| fixture | prod rescued | night-006 rescued | overlap | NEW "
                  "(night-006-only) | regressed (prod-only) |")
        md.append("|---|---|---|---|---|---|")
        for fixture, label in [("t2s_degenerate", "A t2s_degenerate"),
                               ("qwen_degenerate", "B qwen_degenerate")]:
            pv = ctx["provenance_prod"][fixture]
            md.append(f"| {label} | {pv['n_baseline_rescued']} | "
    ...
    if ctx.get("provenance_ds035"):
        ...
        md.append("")
        md.append("For all three fixtures, night-006 rescues are decomposed "
                  "against the DS-035 gated rescued record-id set (REUSED).")
        md.append("")
        md.append("| fixture | DS-035 rescued | night-006 rescued | overlap | "
                  "NEW (night-006-only) | regressed (DS-035-only) |")
        md.append("|---|---|---|---|---|---|")
    ...
    md.append("")
    md.append("**GATE FAIL: t2s_degenerate = 0.13 < 0.15.** The spectral-gated "
              "kickstart never fired on any corpus (0 events across all 350 "
              "records + smoke). The kickstart gate `spectral_fire AND "
              "trailing_ctr < 0.50 AND active_loop_ids` was never satisfiable "
              "because spectral-fire steps co-occur with `trailing_ctr >= 0.50` "
              "on every fixture.")
    md.append("")
    md.append("Per-step evidence (from the night-006 JSONL):")
    md.append("")
    md.append("| fixture | spectral_fire steps | with trailing_ctr < 0.50 |")
    md.append("|---|---|---|")
    md.append(f"| t2s_degenerate | {ctx['kick_analysis']['t2s_spectral_steps']} "
              f"| {ctx['kick_analysis']['t2s_ctr_below']} |")
    md.append(f"| qwen_degenerate | {ctx['kick_analysis']['qwen_spectral_steps']} "
              f"| {ctx['kick_analysis']['qwen_ctr_below']} |")
    md.append(f"| heldout_degenerate_v2 | "
              f"{ctx['kick_analysis']['heldout_spectral_steps']} | "
              f"{ctx['kick_analysis']['heldout_ctr_below']} |")
    md.append(f"| TOTAL | {ctx['kick_analysis']['total_spectral_steps']} | "
              f"{ctx['kick_analysis']['total_ctr_below']} |")
    md.append("")
    md.append("**Root cause.** The spectral-collapse signature (PR < band_low "
              "= 8.216097) co-occurs with healthy surface text (trailing_ctr "
              "≈ 1.0) on all three degenerate fixtures — the same pattern "
              "documented by DS-034c/DS-034e (prose FP records 98/99: PR ≈ "
              "6.6–7.5 with trailing_ctr ≈ 1.0). Requiring `trailing_ctr < "
              "0.50` on the spectral-gated kickstart therefore structurally "
              "blocks the kickstart from ever firing at spectral-fire steps. "
              "This is the mechanism that produced the 11 DS-035 spectral-only "
              "t2s rescues (via the is_collapsed-gated kickstart, 80.2 "
              "events/record); night-006 loses those rescues (11 DS-035-only "
              "regressions, 4 NEW) and stays at the night-004 level (13/100).")
    md.append("")
    md.append("**Secondary finding (heldout_v2 1.00 → 0.96).** night-004 "
              "achieved 50/50 on Fixture C with the production kickstart trigger "
              "(2.08 events/record). The spectral-gated kickstart never fires, "
              "so Fixture C drops to 48/50 — still >= 0.96 (MET), but the "
              "production kickstart contribution is lost.")
    md.append("")
    md.append("**Secondary finding (prose-100).** 0 FPs (MET, <= 7) — the "
              "dual-predicate fired on 0/100 prose records. However, "
              "unconditional suppression changed the continuation of 52/100 "
              "prose records (any ΔD2 > 0); these are not FPs under the "
              "ratified criterion (fire_count = 0) but are a non-trivial "
              "actuation footprint of the production suppression path on "
              "healthy prose.")
    md.append("")
    ...
    md.append("- THE CHANGE (only actuation delta vs night-004): the kickstart "
              "gate is spectral — `spectral_fire AND trailing_ctr < 0.50 AND "
              "active_loop_ids`. Suppression stays UNCONDITIONAL (production "
    ...
    md.append("- DS-033 production and DS-035 gated rescue rates are REUSED "
              "for the comparison tables (do NOT re-run).")
...
def build_ctx_from_results(
    ...
    night-004 unconditional vs night-006 probe vs target), rescue provenance
    against both the DS-033 production and DS-035 gated rescued-id sets, the
    prose-100 FP aggregate, and the DS-034e spectral coverage on the t2s
    detection-gap subset.
    """
    ...
    for fixture in fixture_names:
        ...
        night004_rate = night004_rates.get(fixture, float("nan"))
        night006_rate = rescue_agg[fixture]["rescue_rate"]
        target_rate = {
        ...
        comparison[fixture] = {
            ...
            "delta_vs_prod": float(night006_rate - production_rate),
            "delta_vs_gated": float(night006_rate - gated_rate),
            "delta_vs_night004": float(night006_rate - night004_rate),
        }
    ...
    gate_metrics = {
        "t2s_degenerate": {
            "value": float(comparison["t2s_degenerate"]["night006_rate"]),
            "target": TARGET_T2S,
            "met": bool(comparison["t2s_degenerate"]["night006_rate"]
                        >= TARGET_T2S - 1e-9),
        },
        "qwen_degenerate": {
            "value": float(comparison["qwen_degenerate"]["night006_rate"]),
            "target": TARGET_QWEN,
            "met": bool(comparison["qwen_degenerate"]["night006_rate"]
                        >= TARGET_QWEN - 1e-9),
        },
        "heldout_degenerate_v2": {
            "value": float(comparison["heldout_degenerate_v2"]["night006_rate"]),
            "target": TARGET_HELDOUT_V2,
            "met": bool(comparison["heldout_degenerate_v2"]["night006_rate"]
    ...
    # Kickstart-condition analysis: how many spectral_fire steps had
    # trailing_ctr < 0.50 (the spectral-gated kickstart requirement)?
            ...
            for ev in r["active"].get("fire_events", []):
                if ev.get("spectral_fire"):
                    ...
                    if ev.get("trailing_ctr", 1.0) < CTR_THRESHOLD:
                        ctr_below += 1
        return total_spec, ctr_below

    t2s_spec, t2s_below = _kick_step_stats("t2s_degenerate")
    qwen_spec, qwen_below = _kick_step_stats("qwen_degenerate")
    held_spec, held_below = _kick_step_stats("heldout_degenerate_v2")
    kick_analysis = {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="night-006 controller integration probe — dual-predicate + "
                    "spectral-gated kickstart (GATE)"
    )
    ...
    parser.add_argument("--report-only", action="store_true",
                        help="Regenerate CONTROLLER_INTEGRATION_RESULTS.md "
                             "from an existing "
                             "controller_integration_results.jsonl without "
    ...
    print("=" * 70)
    print("night-006 Controller integration probe (GATE)")
    print("=" * 70)
    print(f"SEED: {SEED}")
    ...
    print("change: UNCONDITIONAL suppression (unchanged) + spectral-gated "
          "kickstart (spectral_fire AND trailing_ctr < 0.50 AND active_loop_ids)")
    ...
    # night-004 unconditional suppression rescue rates (REUSED for comparison).
    night004_rates: Dict[str, float] = {}
    if Path("docs/gate23/unconditional_suppression_results.jsonl").exists():
        n4_rates, _ = load_rescue_rates(
            Path("docs/gate23/unconditional_suppression_results.jsonl"),
            fixtures_all,
        )
        night004_rates = n4_rates
    else:
        print("[warn] unconditional_suppression_results.jsonl not found; "
              "night-004 comparison column will be n/a.")
    ...
    if args.report_only:
        ...
        wall_clock_s: Any = "n/a (report-only regeneration of the night-006 run)"
        ...
        ctx = build_ctx_from_results(
            ...
            ds033_rescued_ids, ds035_rescued_ids, night004_rates,
    ...
    def run_fixture(
        ...
        for rec_i, rec in enumerate(records):
            ...
            # Active (live, night-006 controller integration probe).
    ...
    print("\n" + "=" * 70)
    print("NIGHT-006 CONTROLLER INTEGRATION PROBE")
    for fixture, label in [("t2s_degenerate", "A t2s_degenerate"),
                           ("qwen_degenerate", "B qwen_degenerate"),
                           ("heldout_degenerate_v2", "C heldout_degenerate_v2")]:
        ...
        print(f"    target {g['target']:.4f} -> {'MET' if g['met'] else 'NOT MET'} "
              f"(vs prod {c['ds033_production_rate']:.4f} "
              f"vs gated {c['ds035_gated_rate']:.4f} "
              f"vs n4 {c['night004_rate']:.4f})")
    ...
    if verdict == "FAIL":
        print("[STOP] night-006 GATE: FAIL — one or more targets not met. "
              "Red gate is a STOP signal; no thresholds or gates are "
              "negotiated [1].")
        sys.exit(1)
    print("night-006 GATE: PASS — all four targets met. The proposed "
          "controller edit is validated and can be applied.")
