...
night-006 FAIL: t2s_degenerate = 0.13 < 0.15. Kickstart events = 0 on all
three degenerate fixtures because the spectral-gated kickstart condition
`spectral_fire AND trailing_ctr < 0.50 AND active_loop_ids` never tripped:
t2s macro-loops keep trailing_ctr near 1.0 (text is locally coherent) and
active_loop_ids is sparse (repetition period exceeds the 24-token window).

DS-035 proved spectral_fire alone, gating kickstart, produced 11 new t2s
rescues. The fix is NARROW: change the kickstart trigger from the three-way
AND gate to `spectral_fire` alone. One condition, one line changed.

This probe tests the exact `diagnose()` / `generate()` changes proposed for
`governor/controller.py` BEFORE they reach the controller. It is a
measurement probe implementing the proposed logic inline and validating it on
all three degenerate fixtures + prose-100 FP verification.

THIS IS A GATE. Targets are explicit, pass/fail is binary, a red gate is a
STOP signal [1].

Detection (dual-predicate diagnose() logic, exact; UNCHANGED from night-006):
...
Actuation (generate() changes, exact):
    ...
    # FIXED (night-007): kickstart on spectral_fire alone. Removed the
    # trailing_ctr < 0.50 AND active_loop_ids conditions that killed
    # night-006's t2s coverage (0 kickstart events on every fixture).
    if spectral_fire:
        kickstart_counter = 3

The kickstart trigger is the ONLY actuation change vs night-006. Detection,
unconditional suppression, top_p, and the kickstart schedule are unchanged.
Suppression remains unconditional, matching the production combined path
(controller.py:677-679).

Gate criteria (all four must be met; same as night-006):
...
Reused data (do NOT re-run):
  ...
  - night-006 baseline: REUSED from
    ...
    Do NOT re-run night-006 or dormant.
...
# ---------------------------------------------------------------------------
SEED = 42  # night-007 measurement seed (matches ds-025..ds-035, night-004/006)
...
# Gate criteria (night-007 task file; all four must be met — same as night-006).
...
OUTPUT_JSONL = Path("docs/gate23/narrow_kickstart_results.jsonl")
OUTPUT_MD = Path("docs/gate23/NARROW_KICKSTART_RESULTS.md")
...
# ---------------------------------------------------------------------------
# KV-cache generation: active (night-007 narrow kickstart probe)
# ---------------------------------------------------------------------------
@torch.no_grad()
def greedy_generate_active_controller_integration(
    ...
    """Greedy (do_sample=False) active generation under the night-007 probe.
    ...
    Proposed actuation (generate() changes, exact):
      ...
      - NARROW kickstart (THE NIGHT-007 CHANGE): fires on spectral_fire ALONE
        (counter=3, -1e4/-5.0/-2.0). The trailing_ctr < 0.50 AND active_loop_ids
        conditions that killed night-006's t2s coverage are REMOVED. Detection
        (trailing_ctr / code-context) is still computed for the bigram predicate.
    ...
    try:
        for step in range(max_new_tokens):
            ...
            # ---- 6. Actuation: NARROW kickstart (THE NIGHT-007 CHANGE) ----
            # night-006 FAIL: the three-way gate `spectral_fire AND
            # trailing_ctr < 0.50 AND active_loop_ids` never tripped (0 kickstart
            # events on every fixture). DS-035 proved spectral_fire alone,
            # gating kickstart, produced 11 new t2s rescues. The fix is NARROW:
            # kickstart on spectral_fire ALONE. One condition, one line changed.
            if spectral_fire:
...
def rescue_provenance(
    ...
    """Per-fixture provenance of the night-007 rescues vs a baseline rescue set.

    For each fixture in baseline_rescued:
      - overlap       : records rescued by BOTH the baseline and night-007.
      - new_rescues   : records rescued ONLY by night-007 (baseline missed).
      - regressions   : records rescued by the baseline but NOT by night-007.
    """
...
def run_determinism_smoke(
    ...
    """Determinism smoke (night-007): one Fixture A record.
      1. Dormant (reused from DS-035 for the same record id), checked for the
         smoke only via a live re-run.
      2. Active (night-007 narrow kickstart), generated twice — token ids
...
def run_determinism_smoke_prose(
    ...
    """Determinism smoke (prose-100): one heldout prose record.
      Active (night-007 narrow kickstart) generated twice — token ids AND
...
def write_markdown_report(ctx: Dict[str, Any]) -> None:
    md: List[str] = []
    md.append("# night-007 — Narrow kickstart trigger: spectral_fire alone (GATE)")
    md.append("")
    md.append("> GATE REPORT. night-006 FAIL: t2s_degenerate = 0.13 < 0.15 "
              "because the three-way kickstart gate `spectral_fire AND "
              "trailing_ctr < 0.50 AND active_loop_ids` never tripped. This "
              "probe tests the NARROW fix — kickstart on `spectral_fire` alone — "
              "implementing the exact `diagnose()` / `generate()` changes "
              "proposed for `governor/controller.py` BEFORE they reach the "
              "controller. Targets are explicit, pass/fail is binary, a red "
    ...
    md.append("| Dual-predicate | DS-034e frozen config (proposed diagnose(); "
              "UNCHANGED from night-006): bigram OR spectral, band_low, >=2 "
              "consecutive, token_diversity < 0.40 corroboration on "
    ...
    md.append("| Kickstart | FIXED (night-007): spectral_fire ALONE -> "
              "counter=3 (-1e4/-5.0/-2.0). The trailing_ctr < 0.50 AND "
              "active_loop_ids conditions that killed night-006 are REMOVED. |")
    ...
    # Gate criteria
    md.append("## Gate criteria (quoted from the night-007 task file)")
    md.append("")
    md.append("| fixture | target | rationale |")
    md.append("|---|---|---|")
    md.append("| t2s_degenerate | >= 0.15 | match production; spectral rescues "
              "expected |")
    md.append("| qwen_degenerate | >= 0.65 | match night-004 unconditional "
              "suppression |")
    ...
    md.append("One Fixture A record AND one prose-100 record: active "
              "(night-007 narrow kickstart) generated twice. Token ids AND "
    ...
    md.append("")
    md.append("| fixture | target | night-007 | met? |")
    md.append("|---|---|---|---|")
    ...
    md.append("")
    if verdict == "PASS":
        md.append("**PASS** — all four targets met. The narrow kickstart "
                  "trigger change (spectral_fire alone) is validated and can "
                  "be applied to `governor/controller.py`.")
    else:
    ...
    md.append("## Comparison table (vs night-006)")
    md.append("")
    md.append("| fixture | night-006 | night-007 | target | met? |")
    md.append("|---|---|---|---|---|")
    ...
    md.append("night-006 rates are REUSED from "
              "`docs/gate23/controller_integration_results.jsonl` (do NOT "
    ...
    md.append("Fire counts reflect DUAL-PREDICATE DETECTION liveness. The "
              ...
              "spectral kickstart (now spectral_fire alone).")
    md.append("")
    ...
    md.append("night-006 kickstart events were 0.00 on every fixture (the "
              ...
              "spectral_fire alone is expected to be non-zero on the degenerate "
              "fixtures (Table 6 / kickstart analysis below).")
    md.append("")
    ...
    md.append("## Table 5 — Prose-100 FP verification")
    md.append("")
    p = tabs["prose"]
    ...
    md.append("The ratified RFC-004 A3 FP policy documents 7 FPs (5 bigram "
              ...
              "The gate requires the night-007 probe to not exceed 7 FPs under "
              ...
              "generate abundant repeated bigrams; the spectral-gated kickstart "
    ...
    # Rescue provenance vs DS-035 gated and night-006
    if ctx.get("provenance_ds035"):
        md.append("## Rescue provenance vs DS-035 gated")
        md.append("")
        md.append("For all three fixtures, night-007 rescues are decomposed "
                  "against the DS-035 gated rescued record-id set (REUSED). "
                  "DS-035 gated kickstart on is_collapsed (bigram OR spectral); "
                  "night-007 gates on spectral_fire alone.")
        md.append("")
        md.append("| fixture | DS-035 rescued | night-007 rescued | overlap | "
                  "NEW (night-007-only) | regressed (DS-035-only) |")
        md.append("|---|---|---|---|---|---|")
    ...
    if ctx.get("provenance_night006"):
        ...
        md.append("")
        md.append("| fixture | night-006 rescued | night-007 rescued | overlap | "
                  "NEW (night-007-only) | regressed (night-006-only) |")
        md.append("|---|---|---|---|---|---|")
        for fixture in ("t2s_degenerate", "qwen_degenerate",
                        "heldout_degenerate_v2"):
    ...
    # Kickstart analysis (night-007 trigger)
    md.append("## Table 6 — Kickstart analysis (spectral_fire alone)")
    md.append("")
    md.append("night-007 fires kickstart on spectral_fire ALONE, so "
              "n_kickstart_events == spectral_fire steps by construction. The "
              "`with trailing_ctr < 0.50` column shows how many of those steps "
              "WOULD have satisfied the night-006 gate (the condition that "
              "blocked night-006).")
    md.append("")
    md.append("| fixture | spectral_fire steps | with trailing_ctr < 0.50 | "
              "kickstart events |")
    md.append("|---|---|---|---|")
    ka = ctx["kick_analysis"]
    md.append(f"| t2s_degenerate | {ka['t2s_spectral_steps']} | "
              f"{ka['t2s_ctr_below']} | {ka['t2s_kick_events']} |")
    md.append(f"| qwen_degenerate | {ka['qwen_spectral_steps']} | "
              f"{ka['qwen_ctr_below']} | {ka['qwen_kick_events']} |")
    md.append(f"| heldout_degenerate_v2 | {ka['heldout_spectral_steps']} | "
              f"{ka['heldout_ctr_below']} | {ka['heldout_kick_events']} |")
    md.append(f"| TOTAL | {ka['total_spectral_steps']} | "
              f"{ka['total_ctr_below']} | {ka['total_kick_events']} |")
    md.append("")
    ...
    md.append("")
    if verdict == "PASS":
        md.append("**GATE PASS.** The narrow kickstart trigger change "
                  "(spectral_fire alone) restores the spectral kickstart that "
                  "night-006's three-way gate suppressed. Kickstart events are "
                  "non-zero on all three degenerate fixtures (Table 6), and the "
                  "t2s blind-spot target is recovered (>= 0.15).")
    else:
    ...
    md.append("**night-006 root cause (confirmed).** The spectral-collapse "
              ...
              "trailing_ctr < 0.50).")
    md.append("")
    md.append("**The night-007 fix (verified).** The three-way AND gate is "
              "replaced by `spectral_fire` alone. Every spectral-fire step now "
              "triggers kickstart (counter=3, -1e4/-5.0/-2.0). This is the "
              "mechanism that produced the 11 DS-035 spectral-only t2s rescues.")
    md.append("")
    if verdict == "FAIL":
        md.append("**GATE FAIL root cause — qwen regression (0.65 -> 0.61).** "
                  "night-007 loses 4 night-006 qwen rescues (record_ids 22, 23, "
                  "24, 25) with NO new qwen rescues (0 new). On those records "
                  "the spectral kickstart fires at step 108 (20 events each; "
                  "detection is identical to night-006 — first fire at the same "
                  "step, PR, token_diversity, trailing_ctr), and the -1e4 "
                  "non-prose kickstart penalty at counter=3 disrupts the "
                  "suppression-only rescue that night-006 achieved (deltaD2 "
                  "collapses from +0.2174 to 0.0000, i.e. back to the dormant "
                  "degenerate loop). The aggressive spectral kickstart is "
                  "counterproductive on this qwen subset: 0 of the 3524 qwen "
                  "spectral-fire steps satisfied the night-006 trailing_ctr < "
                  "0.50 gate, so night-006's suppression-only path (which "
                  "rescued 65/100) is replaced by a kickstart path that "
                  "rescues only 61/100.")
        md.append("")
        md.append("**t2s / heldout / prose (MET).** t2s recovers 13 -> 24 "
                  "(the 11 DS-035 spectral-only rescues are recovered exactly: "
                  "17, 21, 23, 31, 35, 42, 50, 53, 85, 88, 93; 0 regressions "
                  "vs night-006). heldout_v2 recovers 48 -> 50 (1.00, matching "
                  "night-004). prose-100 stays at 0 FPs (dual-predicate fired "
                  "on 0/100; spectral kickstart dormant by construction).")
        md.append("")
    md.append("**Prose-100 FP risk (verified).** Unconditional suppression does "
              ...
              "spectral-gated kickstart is dormant on prose-100 by "
              "construction, so the kickstart trigger change does not add FPs "
              "to healthy prose.")
    md.append("")
    ...
    md.append("- THE CHANGE (only actuation delta vs night-006): the kickstart "
              "gate is `spectral_fire` ALONE — `spectral_fire AND "
              "trailing_ctr < 0.50 AND active_loop_ids` -> `spectral_fire`. "
              "Suppression stays UNCONDITIONAL (production "
              "controller.py:677-679 coupling), which night-004 established "
              "bridges the qwen actuation gap (0.20 -> 0.65).")
    ...
    md.append("- Dormant baselines are REUSED (do NOT re-run): Fixtures A/B "
              ...
              "fallback_hardened).")
    md.append("- night-006 baseline rates and rescued-id sets are REUSED from "
              "docs/gate23/controller_integration_results.jsonl (do NOT "
              "re-run). DS-035 gated rates/rescued-ids are REUSED from "
...
def build_ctx_from_results(
    ...
    Computes the comparison table (night-006 baseline vs night-007 probe vs
    target), rescue provenance against both the DS-035 gated and night-006
    rescued-id sets, the prose-100 FP aggregate, and the DS-034e spectral
    coverage on the t2s detection-gap subset.
    """
    ...
    for fixture in fixture_names:
        ...
        night007_rate = rescue_agg[fixture]["rescue_rate"]
        ...
        comparison[fixture] = {
            ...
            "delta_vs_night006": float(night007_rate - night006_rate),
            "delta_vs_ds035": float(night007_rate - ds035_gated_rate),
        }
    ...
    gate_metrics = {
        "t2s_degenerate": {
            "value": float(comparison["t2s_degenerate"]["night007_rate"]),
            "target": TARGET_T2S,
            "met": bool(comparison["t2s_degenerate"]["night007_rate"]
                        >= TARGET_T2S - 1e-9),
        },
        "qwen_degenerate": {
            "value": float(comparison["qwen_degenerate"]["night007_rate"]),
            "target": TARGET_QWEN,
            "met": bool(comparison["qwen_degenerate"]["night007_rate"]
                        >= TARGET_QWEN - 1e-9),
        },
        "heldout_degenerate_v2": {
            "value": float(comparison["heldout_degenerate_v2"]["night007_rate"]),
            "target": TARGET_HELDOUT_V2,
            "met": bool(comparison["heldout_degenerate_v2"]["night007_rate"]
    ...
    # Kickstart-condition analysis: how many spectral_fire steps fire per
    # fixture, and how many of those would ALSO have satisfied the night-006
    # trailing_ctr < 0.50 gate (the condition that blocked night-006)?
    # night-007 fires kickstart on spectral_fire alone, so n_kickstart_events
    # == spectral_fire steps by construction.
    def _kick_step_stats(fx: str) -> Tuple[int, int, int]:
        total_spec = 0
        ctr_below = 0
        kick_events = 0
        for r in all_results:
            ...
            for ev in r["active"].get("fire_events", []):
                if ev.get("spectral_fire"):
                    total_spec += 1
                    if ev.get("trailing_ctr", 1.0) < CTR_THRESHOLD:
                        ctr_below += 1
            kick_events += int(r["active"]["n_kickstart_events"])
        return total_spec, ctr_below, kick_events

    t2s_spec, t2s_below, t2s_kick = _kick_step_stats("t2s_degenerate")
    qwen_spec, qwen_below, qwen_kick = _kick_step_stats("qwen_degenerate")
    held_spec, held_below, held_kick = _kick_step_stats("heldout_degenerate_v2")
    kick_analysis = {
        "t2s_spectral_steps": t2s_spec,
        "t2s_ctr_below": t2s_below,
        "t2s_kick_events": t2s_kick,
        "qwen_spectral_steps": qwen_spec,
        "qwen_ctr_below": qwen_below,
        "qwen_kick_events": qwen_kick,
        "heldout_spectral_steps": held_spec,
        "heldout_ctr_below": held_below,
        "heldout_kick_events": held_kick,
        "total_spectral_steps": t2s_spec + qwen_spec + held_spec,
        "total_ctr_below": t2s_below + qwen_below + held_below,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="night-007 narrow kickstart probe — spectral_fire alone "
                    "(GATE)"
    )
    ...
    parser.add_argument("--report-only", action="store_true",
                        help="Regenerate NARROW_KICKSTART_RESULTS.md "
                             "from an existing "
                             "narrow_kickstart_results.jsonl without "
    ...
    print("=" * 70)
    print("night-007 Narrow kickstart probe (GATE)")
    print("=" * 70)
    print(f"SEED: {SEED}")
    ...
    print("change: UNCONDITIONAL suppression (unchanged) + kickstart on "
          "spectral_fire ALONE (removed trailing_ctr < 0.50 AND active_loop_ids)")
    ...
    if args.report_only:
        ...
        wall_clock_s: Any = "n/a (report-only regeneration of the night-007 run)"
        ...
        ctx = build_ctx_from_results(
            all_results, ds035_rates, night006_rates,
            ds035_rescued_ids, night006_rescued_ids,
    ...
    def run_fixture(
        ...
        for rec_i, rec in enumerate(records):
            ...
            # Active (live, night-007 narrow kickstart probe).
    ...
    print("\n" + "=" * 70)
    print("NIGHT-007 NARROW KICKSTART PROBE")
    for fixture, label in [("t2s_degenerate", "A t2s_degenerate"),
                           ("qwen_degenerate", "B qwen_degenerate"),
                           ("heldout_degenerate_v2", "C heldout_degenerate_v2")]:
    ...
    if verdict == "FAIL":
        print("[STOP] night-007 GATE: FAIL — one or more targets not met. "
              "Red gate is a STOP signal; no thresholds or gates are "
              "negotiated [1].")
        sys.exit(1)
    print("night-007 GATE: PASS — all four targets met. The narrow kickstart "
          "trigger change (spectral_fire alone) is validated and can be "
          "applied.")
