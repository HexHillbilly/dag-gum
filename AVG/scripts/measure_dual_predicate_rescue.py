...
RFC-004 Amendment A3 (ratified with FP policy) adds spectral PR detection
(layer 2, band_low = 8.216097, >=2 consecutive steps, token_diversity < 0.40
corroboration on spectral-only fires) to the production bigram predicate.
DS-033 proved 93% detection-gap on t2s_degenerate under bigram-only. DS-034
proved 100% spectral PR liveness on those blind spots. DS-034e confirmed
92/93 spectral fire coverage with token_diversity corroboration while
accepting 7 documented prose-100 FPs as pre-production debt.

THIS IS THE GATE. The dual-predicate's entire value proposition is rescue
efficacy. If it rescues degeneracy that bigram misses, the FP tax is
justified. If it rescues zero, the amendment dies.

Owner directive (independent review):
  - Rescue >= 1 on Fixture A (t2s_degenerate) OR Fixture B (qwen_degenerate)
    -> PASS. A3 ratifies.
  - Rescue = 0 on BOTH -> FAIL. A3 auto-reverts to bigram-only and the
    dual-predicate is shelved.
  - Fixture C (heldout_degenerate_v2 positive control): rescue rate >= 85/100
    (no regression; suppression-only baseline is 48/50 = DS-031).

Frozen dual-predicate rule (DS-034e configuration):
...
Actuation: production logit-penalty path (token suppression cooldown=8
penalty=-5.0, kickstart counter=3, top_p=0.85). NO residual hooks.
...
Fixtures:
  A: tests/fixtures/t2s_degenerate.jsonl (night-001; N=100). record["text"]
     ...
     DS-034e: 92/93 spectral fire coverage).
  B: tests/fixtures/qwen_degenerate.jsonl (ds-004; N=100). record["prompt"]
     as prompt. Secondary blind-spot target (DS-033: 73% detection-gap).
  C: tests/fixtures/heldout_degenerate_v2.jsonl (ds-027; N=100). 50-record
     ...
     record["mutated_prompt"] as prompt. Positive control (DS-031:
     suppression-only rescues 48/50).

Reused data (do NOT re-run):
  - DS-028 dormant for Fixture C:
    docs/gate23/layer_effect_persistent_results.jsonl (join on record_id,
    layer==2). 50 dormant baselines.
  - DS-033 production baseline rescue rates for Fixtures A/B:
    docs/gate23/production_cross_fixture_results.jsonl.
  - DS-031 production baseline for Fixture C: 48/50 (suppression-only,
    documented in the DS-035 task file; DS-033 did not cover Fixture C).

Environment notes (identical to ds-025/ds-033/ds-034c):
...
# Gate criteria (owner directive / DS-035 task file).
FIXTURE_C_RESCUE_RATE_THRESHOLD = 0.85  # >= 85/100 positive-control floor
...
OUTPUT_JSONL = Path("docs/gate23/dual_predicate_rescue_results.jsonl")
OUTPUT_MD = Path("docs/gate23/DUAL_PREDICATE_RESCUE_RESULTS.md")
...
def greedy_generate_active_dual(
    ...
    """Greedy (do_sample=False) active generation under the DS-034e dual-predicate.
    ...
    Frozen DS-034e dual-predicate rule (exact):
        bigram_collapse   = (token_diversity < 0.30) AND (trailing_ctr < 0.30)
        if bigram_collapse: bigram_ctr += 1 else: bigram_ctr = 0
        bigram_fire       = bigram_ctr >= 2
    ...
    cooldown=8 penalty=-5.0, kickstart counter=3, top_p=0.85). NO residual
    hooks. Every fire is logged with (step, which predicate(s) triggered, PR).
    """
    enc = tokenizer(prompt, return_tensors="pt").to(model.device)
    prompt_ids = enc["input_ids"]
    prompt_len = int(prompt_ids.shape[-1])
...
def run_determinism_smoke(
    ...
    """Determinism smoke (DS-035): one Fixture A record.
...
def write_markdown_report(ctx: Dict[str, Any]) -> None:
    md: List[str] = []
    md.append("# DS-035 — Dual-predicate cross-fixture rescue gate (GATE)")
    md.append("")
    md.append("> GATE REPORT (RFC-004 Amendment A3, owner directive). The "
              "dual-predicate's entire value proposition is rescue efficacy. "
              "Rescue >= 1 on Fixture A or B ratifies A3. Rescue = 0 on both "
              "auto-reverts to bigram-only and the dual-predicate is shelved.")
    md.append("")
    ...
    md.append("| Dormant | LIVE (greedy, KV-cache) for Fixtures A/B; REUSED "
              "from DS-028 (layer_effect_persistent_results.jsonl, layer==2, "
              "join on record_id) for Fixture C |")
    md.append("| DS-033 baselines | REUSED from "
              "production_cross_fixture_results.jsonl (do NOT re-run) |")
    ...
    # Gate criteria
    md.append("## Gate criteria (quoted from the DS-035 task file / owner "
              "directive)")
    md.append("")
    md.append("> - Rescue ≥ 1 on Fixture A or B → PASS. A3 ratifies.")
    md.append("> - Rescue = 0 on BOTH → FAIL. A3 auto-reverts to bigram-only "
              "[1].")
    md.append("> - Fixture C: rescue rate ≥ 85/100 (positive control — no "
              "regression).")
    md.append("")
    ...
    md.append("One Fixture A record: dormant generated twice AND active "
              "(DS-034e dual-predicate) generated twice. Token ids AND the "
    ...
    for fixture in ("t2s_degenerate", "qwen_degenerate", "heldout_degenerate_v2"):
        ...
        md.append(f"| {fixture} rescue count | {a['n_rescue']}/{a['n_records']} "
                  f"({fmt(a['rescue_rate'] * 100, 2)}%) |")
    md.append(f"| Fixture C rescue rate ≥ 85% | "
              f"{'PASS' if tabs['fixture_c_ok'] else 'FAIL'} |")
    md.append(f"| **Gate verdict** | **{tabs['verdict']}** |")
    md.append("")
    if tabs["verdict"] == "PASS":
        md.append("**PASS** — rescue count ≥ 1 on Fixture A or B. Amendment A3 "
                  "ratifies; the dual-predicate rescue efficacy is established. "
                  "FP optimization becomes the next night gate.")
    else:
        md.append("**FAIL** — rescue count = 0 on both t2s_degenerate AND "
                  "qwen_degenerate. RFC-004 Amendment A3 auto-reverts to "
                  "bigram-only and the dual-predicate is shelved. STOP signal; "
    ...
    md.append("")
    md.append("| fixture | production rescue rate [DS-033] | dual-predicate "
              "rescue rate | delta |")
    md.append("|---|---|---|---|")
    for fixture in ("t2s_degenerate", "qwen_degenerate", "heldout_degenerate_v2"):
        c = tabs["comparison"][fixture]
        md.append(f"| {fixture} | {fmt(c['production_rate'], 4)} | "
                  f"{fmt(c['dual_predicate_rate'], 4)} | "
                  f"{fmt(c['delta'], 4)} |")
    md.append("")
    md.append("DS-033 production baseline rescue rates for Fixtures A/B are "
              "REUSED from `docs/gate23/production_cross_fixture_results.jsonl` "
              "(do NOT re-run). DS-033 did not cover Fixture C; the Fixture C "
              "production baseline is the DS-031 suppression-only 48/50 = 0.9600 "
              "(documented in the DS-035 task file).")
    md.append("")
    ...
    # Fire-type counts
    md.append("## Table 2 — Fire-type counts per fixture (active)")
    md.append("")
    ...
    md.append("")
    md.append("A record can have both bigram and spectral fires (possibly at "
              "different steps); the 'both-fire records' column counts records "
              "with at least one step where both fired simultaneously.")
    md.append("")

    # Dormant reference
    md.append("## Table 3 — Dormant reference (per fixture)")
    md.append("")
    ...
    if ctx.get("provenance"):
        ...
        md.append("")
        md.append("For Fixtures A/B, the dual-predicate rescues are decomposed "
                  "against the DS-033 production rescued record-id set "
                  "(REUSED from production_cross_fixture_results.jsonl).")
        md.append("")
        md.append("| fixture | prod rescued | dual rescued | overlap | NEW "
                  "(dual-only) | regressed (prod-only) |")
        md.append("|---|---|---|---|---|---|")
        for fixture, label in [("t2s_degenerate", "A t2s_degenerate"),
                               ("qwen_degenerate", "B qwen_degenerate")]:
            p = ctx["provenance"][fixture]
        ...
        for fixture, label in [("t2s_degenerate", "A t2s_degenerate"),
                               ("qwen_degenerate", "B qwen_degenerate")]:
            p = ctx["provenance"][fixture]
            md.append(f"**{label}** — new (dual-only) rescues: "
                      f"`{p['new_ids']}`; regressed (production-only): "
                      f"`{p['regressed_ids']}`.")
            md.append("")
        md.append("NEW rescues are records the production bigram-only path "
                  "missed but the dual-predicate rescued; these are the "
                  "dual-predicate's incremental rescue efficacy.")
        md.append("")
    ...
    if ctx.get("t2s_coverage"):
        ...
        md.append(f"Of the {cov['n_detection_gap']} DS-033 t2s_degenerate "
                  ...
                  f"{cov['rate']:.1%} coverage. DS-034e reported 92/93; this "
                  f"run confirms the spectral liveness on the primary blind-spot "
                  f"target.")
        md.append("")
    ...
    md.append("- GATE: the pass/fail criterion is explicit and quoted from the "
              "task file. Rescue = ΔDistinct-2 > 0 (DS-033 convention). A FAIL "
              "is a STOP signal — the script exits non-zero; no thresholds are "
              "negotiated or tuned.")
    md.append("- The frozen DS-034e dual-predicate rule is implemented exactly "
              ...
              "spectral-only fires require token_diversity < 0.40 corroboration "
              "when bigram_ctr < 2, and is_code_syntax_context forces "
              "spectral_fire = False.")
    md.append("- Actuation on is_collapsed uses the production logit-penalty "
              ...
              "counter=3 with -1e4/-5.0/-2.0, top_p=0.85). NO residual hooks.")
    ...
    md.append("- Fixture A/B dormant baselines are generated LIVE (greedy, "
              "KV-cache, no hooks/penalties). Fixture C dormant is REUSED from "
              "DS-028 (layer_effect_persistent_results.jsonl, layer==2, join on "
              "record_id) — the 50-record seeded subset matches exactly.")
    md.append("- DS-033 production baseline rescue rates for Fixtures A/B are "
              "REUSED from production_cross_fixture_results.jsonl. Fixture C "
              "production baseline is 48/50 (DS-031 suppression-only; DS-033 "
              "did not cover Fixture C).")
    md.append("- **qwen_degenerate regression (0.68 → 0.20).** The dual-predicate "
              "actuation is gated by is_collapsed: suppression tokens are added "
              "to the cooldown dict only when the collapse predicate fires, "
              "whereas the DS-033 production combined path adds loop-token "
              "suppression at every step where active_loop_ids is non-empty. "
              "On qwen_degenerate this more conservative actuation rescues "
              "fewer records (20 vs 68). All 20 dual rescues overlap with "
              "production; 0 new rescues on qwen. The 11 new spectral rescues "
              "are on t2s_degenerate (the primary blind-spot target).")
    md.append("- **t2s_degenerate improvement (0.15 → 0.20).** The dual-predicate "
              "adds 11 new rescues (records 17, 21, 23, 31, 35, 42, 50, 53, "
              "85, 88, 93) that the production bigram-only path missed — all "
              "spectral-only fires. This is the incremental rescue efficacy the "
...
def build_ctx_from_results(
    ...
    Also computes the DS-034e coverage validation on the t2s detection-gap
    subset and the rescue provenance vs DS-033 production rescued-id sets.
    """
    # DS-034e coverage validation: spectral fires on DS-033 t2s detection-gap
    # records (bigram never fired in DS-033 production). This confirms the
    # DS-034e 92/93 coverage claim on the primary blind-spot target.
    ...
    for fixture in fixture_names:
        ...
        # Production baseline (DS-033 for A/B; DS-031 48/50 for C).
        ...
        dual_rate = rescue_agg[fixture]["rescue_rate"]
        comparison[fixture] = {
            "production_rate": float(production_rate),
            "dual_predicate_rate": float(dual_rate),
            "delta": float(dual_rate - production_rate),
        }

    rescue_a = rescue_agg["t2s_degenerate"]["n_rescue"]
    rescue_b = rescue_agg["qwen_degenerate"]["n_rescue"]
    rescue_c = rescue_agg["heldout_degenerate_v2"]["rescue_rate"]
    fixture_c_ok = rescue_c >= FIXTURE_C_RESCUE_RATE_THRESHOLD
    if rescue_a >= 1 or rescue_b >= 1:
        verdict = "PASS"
    else:
        verdict = "FAIL"
    ...
    provenance = rescue_provenance(all_results, ds033_rescued_ids)
    ...
    return {
        ...
        "gate_metrics": {
            "rescue_a": rescue_a,
            "rescue_b": rescue_b,
            "rescue_c": int(rescue_agg["heldout_degenerate_v2"]["n_rescue"]),
            "fixture_c_ok": fixture_c_ok,
            "verdict": verdict,
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DS-035 dual-predicate cross-fixture rescue gate (GATE, "
                    "RFC A3 owner directive)"
    )
    ...
    parser.add_argument("--report-only", action="store_true",
                        help="Regenerate DUAL_PREDICATE_RESCUE_RESULTS.md from "
                             "an existing dual_predicate_rescue_results.jsonl "
    ...
    print("=" * 70)
    print("DS-035 Dual-predicate cross-fixture rescue gate (GATE)")
    print("=" * 70)
    print(f"SEED: {SEED}")
    ...
    print(f"gate: rescue >= 1 on Fixture A or B -> PASS (A3 ratifies); "
          f"rescue = 0 on both -> FAIL (A3 auto-reverts); "
          f"Fixture C rescue rate >= 85%")
    ...
    if not DS033_JSONL.exists():
        ...
        sys.exit(1)
    ds033_rates, ds033_rescued_ids = load_ds033_rescue_baseline(DS033_JSONL)
    ...
    if args.report_only:
        ...
        wall_clock_s: Any = "n/a (report-only regeneration of the DS-035 run)"
        ...
        ctx = build_ctx_from_results(
            all_results, ds033_rates, ds033_rescued_ids,
            device, dtype, smoke, spectral_threshold,
            wall_clock_s=wall_clock_s,
        )
        write_markdown_report(ctx)
        gm = ctx["gate_metrics"]
        print(f"wrote {OUTPUT_MD} (report-only) — rescue A={gm['rescue_a']} "
              f"B={gm['rescue_b']} C={gm['rescue_c']} "
    ...
    def run_fixture(
        ...
        non_prose_ids: Set[int],
        spectral_threshold: float,
        dormant_reuse: Optional[Dict[int, Dict[str, Any]]] = None,
    ) -> List[Dict[str, Any]]:
        ...
        for rec_i, rec in enumerate(records):
            ...
            # Dormant: live greedy for A/B; REUSED from DS-028 for C.
            if dormant_reuse is not None:
                if rid not in dormant_reuse:
                    raise SystemExit(
                        f"[STOP] Fixture C record_id {rid} not found in DS-028 "
                        f"layer-2 dormant. Cannot reuse."
                    )
                dormant = build_dormant_meta_reused(
                    tokenizer, dormant_reuse[rid],
                    "reused from DS-028 (layer_effect_persistent_results.jsonl)",
                )
            else:
                gen_dorm = greedy_generate_dormant_kv(model, tokenizer, prompt)
                dormant = build_dormant_meta(tokenizer, gen_dorm)

            # Active (live, DS-034e dual-predicate + production penalty path).
    ...
    try:
        ...
        part_a = run_fixture(
            ...
            tokenizer, non_prose_ids, spectral_threshold,
        )
    ...
    gm = ctx["gate_metrics"]
    rescue_a = gm["rescue_a"]
    rescue_b = gm["rescue_b"]
    rescue_c = gm["rescue_c"]
    fixture_c_ok = gm["fixture_c_ok"]
    verdict = gm["verdict"]
    if not fixture_c_ok:
        # Positive-control regression is a red signal even if A/B pass.
        print(f"[WARN] Fixture C rescue rate "
              f"{rescue_agg['heldout_degenerate_v2']['rescue_rate']:.4f} < "
              f"{FIXTURE_C_RESCUE_RATE_THRESHOLD:.2f} (positive-control "
              f"regression).")

    print("\n" + "=" * 70)
    print("DUAL-PREDICATE RESCUE GATE")
    for fixture, label in [("t2s_degenerate", "A t2s_degenerate"),
                           ("qwen_degenerate", "B qwen_degenerate"),
                           ("heldout_degenerate_v2", "C heldout_degenerate_v2")]:
    ...
    print(f"  Fixture C >= 85%: {fixture_c_ok}")
    ...
    if verdict == "FAIL":
        print("[STOP] DS-035 GATE: FAIL — rescue count = 0 on both "
              "t2s_degenerate AND qwen_degenerate. RFC-004 Amendment A3 "
              "auto-reverts to bigram-only; the dual-predicate is shelved.")
        sys.exit(1)
    if not fixture_c_ok:
        rescue_c_rate = rescue_agg["heldout_degenerate_v2"]["rescue_rate"]
        print("[STOP] DS-035 GATE: Fixture C positive-control regression "
              f"(rescue rate {rescue_c_rate:.4f} < "
              f"{FIXTURE_C_RESCUE_RATE_THRESHOLD:.2f}). STOP and report.")
        sys.exit(1)
    print(f"DS-035 GATE: {verdict} — measurement complete")
